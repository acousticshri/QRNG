...
# Create the extraction tab
    ...
    def execute_extraction(self, extraction_method):
        try:
            ...
            # Debug: Print the selected method
            print(f"Selected Extraction Method: {extraction_method}")

            for i, random_numbers in enumerate(self.random_numbers_list):
                file_name = self.file_names[i]
                result_text += f"Results for {file_name}:\n"

                # Normalize the method name for comparison
                normalized_method = extraction_method.strip().lower()

                if normalized_method == "von neumann extraction".lower():
                    extracted_data = self.von_neumann_extraction(random_numbers)
                elif normalized_method == "trevisan extraction".lower():
                    # Ask the user for additional inputs
                    source_file = self.file_add[i]
                    seed_file = filedialog.askopenfilename(title="Select Seed File",
                                                           filetypes=[("Text Files", "*.txt")])
                    if not seed_file:
                        raise Exception("Seed file not selected.")

                    alpha = float(simpledialog.askstring("Min Entropy", "Enter min-entropy per bit (0 < alpha ≤ 1):"))
                    epsilon = float(simpledialog.askstring("Error Tolerance", "Enter desired error (0 < epsilon ≤ 1):"))
                    design = int(
                        simpledialog.askstring("Weak Design", "Enter 0 for standard or 1 for block weak design:"))

                    # Run the Trevisan extraction
                    extracted_data = self.run_trevisan_extraction(source_file, seed_file, alpha, epsilon, design)
                else:
    ...
    def run_trevisan_extraction(self, source_path, seed_path, alpha, epsilon, design):
        try:
            # Define paths to the Trevisan C files
            trevisan_path = "C:/Users/Rajat/Desktop/Project/Python Project/Trevisian Extraction/trevisan.c"
            extractor_path = "C:/Users/Rajat/Desktop/Project/Python Project/Trevisian Extraction/extractor.c"

            # Compile the C program
            compile_command = ["gcc", extractor_path, trevisan_path, "-o", "trevisan_extractor", "-lm"]
